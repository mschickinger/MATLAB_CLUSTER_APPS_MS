...
for idx = 1:length(ch2)
    ...
    jump_intervals = [tmp-100 tmp+99];

    %% Set parameters
    mov.N_read = 200;
    corr_traces = cell(size(jump_intervals,1),1);
    %% Load frames and compute xcorrelation
    for i = 1:size(jump_intervals,1)
        display(['Finding jump in interval ' num2str(i) ' of ' num2str(size(jump_intervals,1))])

        mov.counter = jump_intervals(i,1);
        [cur_mov, frames, go_on]  = mov.readNext;
        cur_mov = cur_mov(49:464,49:464,:);
        corr_traces{i} = zeros(size(cur_mov,3),4);
        ...
        for n=1:size(cur_mov,3)
            ...
            [v,ind]=max(tmp(:));
            [corr_traces{i}(n,2), corr_traces{i}(n,3)] = ind2sub(size(tmp),ind);
            corr_traces{i}(n,1) = v;
            corr_traces{i}(n,4) = corr2(cur_mov(:,:,n), prev_img);
            prev_img = cur_mov(:,:,n);
        ...
        close(h)
        for j = 2:3
            corr_traces{i}(:,j) = corr_traces{i}(:,j) - corr_traces{i}(1,j);
        end
    end

    %% Locate jumps
    jumps{idx} = zeros(size(jump_intervals,1),1);
    for i = 1:size(jump_intervals,1)
        [~, ind] = min(corr_traces{i}(:,4));
        jumps{idx}(i) = jump_intervals(i,1) + ind -1;
    end
end
...
for idx = 1:length(jumps)
    for i = 1:length(jumps{idx})
        subplot(2,1,1)
        hold off
        plot(jump_intervals(i,1):jump_intervals(i,2), corr_traces{i}(:,1), 'k', 'Linewidth', 1)
        hold on
        plot(jump_intervals(i,1):jump_intervals(i,2), corr_traces{i}(:,4), 'r', 'Linewidth', 1)
        plot(jumps{idx}(i), corr_traces{i}(jumps{idx}(i)-jump_intervals(i,1)+1,4), 'bo', 'Linewidth', 1)
        xlabel('Frame'), ylabel('Correlation Coefficient')
        set(gca, 'YLim', [0 1])
        title(['Jump number ' num2str(i) ' out of ' num2str(length(jumps{idx}))],'FontSize', 14)

        subplot(2, 1, 2)
        plot(jump_intervals(i,1):jump_intervals(i,2), corr_traces{i}(:,2), 'g', jump_intervals(i,1):jump_intervals(i,2), corr_traces{i}(:,3), 'b', 'Linewidth', 1)
